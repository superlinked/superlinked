...
class AdminFields:
    def __init__(self) -> None:
        ...
        self.object_blob = AdminFieldDescriptor(
            Field(FieldDataType.BLOB, StorageNaming.OBJECT_BLOB_INDEX_NAME), True, False
        )
    ...
    def create_object_blob_field_data(self, object_blob: Json) -> FieldData | None:
        field_data = self.__create_admin_field_data(
            {
                self.object_blob: json.dumps(object_blob, ensure_ascii=True),
            }
        )
    ...
    def __create_admin_field_data(
        self,
        value_by_admin_field: dict[AdminFieldDescriptor, PythonTypes | None],
    ) -> Sequence[FieldData]:
        return [
            admin_field_data
    ...
    def extract_object_blob_field_data(
        self, field_data: dict[str, FieldData]
    ) -> dict[str, Any] | None:
        if object_blob_str := self.object_blob.extract_value(field_data, str):
            return json.loads(cast(str, object_blob_str))
        return None
